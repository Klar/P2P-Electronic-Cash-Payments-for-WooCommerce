...
abstract class BitcoinCronJob {
	...
	private function get_electrum_util() {
	...
	private function process_payments() {
		...
		$funds_received_value_expires_in_secs = 0;// $this->ecp_settings['funds_received_value_expires_in_mins'] * 60;
		$assigned_address_expires_in_secs     = $this->ecp_settings['assigned_address_expires_in_mins'] * 60;
		$confirmations_required               = $this->ecp_settings['confs_num'];
		...
		if ( is_array( $rows_for_balance_check ) ) {
			...
			foreach ( $rows_for_balance_check as $row_for_balance_check ) {
				...
				$balance_info_array                   = $this->get_electrum_util()->getreceivedbyaddress_info( $address_request_array );

				// Prepare 'address_meta' for use.
				$address_meta    = ECP_unserialize_address_meta( @$row_for_balance_check['address_meta'] );
					...
					*/
					...
					if ( $balance_info_array['balance'] >= $last_order_info['order_total'] ) {
						...
						*/
						...
						ECP__log_event( __FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['btc_address']}' (BTC '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ..." );
						...
						ECP__process_payment_completed_for_order( $last_order_info['order_id'], $balance_info_array['balance'] );
						...
						$query    =
							"UPDATE `$btc_addresses_table_name`
                             ...
                             WHERE `id`='$row_id';";
						$ret_code = $wpdb->query( $query );
						ECP__log_event( __FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed." );
						...
						// if (!$hardcron && $ran_cycles >= $ecp_settings['soft_cron_max_loops_per_run'])
						// {
						// return;
						// }
					}
				} else {
					ECP__log_event( __FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['btc_address']}: " . $balance_info_array['message'] );
				}
				// ..//
			}
		}
	...
	private function pre_generate_addresses() {
		...
		for ( ; $total_unused_addresses < $this->get_max_unused_addresses_buffer(); $total_unused_addresses++ ) {
			$address_data = $this->get_electrum_util()->generate_new_bitcoin_address_for_electrum_wallet();
		}
	}
}
